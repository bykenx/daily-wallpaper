...
func OpenDB() {
	var err error
	db, err = gorm.Open(sqlite.Open(filepath.Join(config.AppHome, "app.db")), &gorm.Config{})
	...
	if db.Migrator().HasTable("download_histories") {
		db.Migrator().RenameTable("download_histories", &ImageItem{})
	}
	err = db.AutoMigrate(&ImageItem{})
